...
\begin{equation*}
  ...
  \end{bmatrix},
\end{equation*}
which is a matrix of dimension $(\mathcal{S}\times 2K)$. The constraint values are given by the point-identified estimands:
\begin{equation*}
  b_{eq} =
  \begin{bmatrix}
...
With $|\mathcal{S}| = 1$, that is a single point-identified estimand, we have an objective of the following form:
\begin{equation*}
  \inf_m |X_1|
\end{equation*}
...
Hence, we now have $2K + |\mathcal{S}|$ choice variables. Denote $S = |\mathcal{S}|$ for brevity.
...
\begin{equation*}
  c =
  \begin{bmatrix}
     \hat{\Gamma}_0^*(b_{01}) & \cdots & \hat{\Gamma}_0^*(b_{0K}) & \hat{\Gamma}_1^*(b_{11}) & \cdots & \hat{\Gamma}_1^*(b_{1K})
  \end{bmatrix}'.
\end{equation*}
Both of these are vectors of length $2K$.
