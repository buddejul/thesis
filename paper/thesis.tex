...
I focus on the coverage and length around the kink. Note that away from the kinkcoverage should converge to $0.9$ since I use confidence intervals using $c_\alpha$ critival values (i.e.\ for a one-sided test).
...
Beyond the conditions required for asmymptotic theory, namely that the subsample size needs to diverge, but no faster than $N$ so that $B/N \to 0$, little further guidance seems to be available.
...
\begin{figure}
  ...
  \caption{Simulation Results for the Binary-IV Model with Bernstein Polynomial MTRs --- Imbens-Manski Critival Values}\label{app_fig:sims_add_alpha_im_crit}
  \centering
