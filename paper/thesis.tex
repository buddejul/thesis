...
% TODO - Add explanation of their sharp set to Section 2?
...
% TODO Check: After presenting the general MTE model setup, the rest of the paper is concerned with presenting inference approaches, that can potentially address the previously raised concerns and comparing their performance in a Monte Carlo simulation.

\section{Simulation Studies}\label{sec:simulation_studies}
% TODO Write intro: Focus on more interesting example. Explain why only bootstrap and subsampling.
...
\begin{figure}
  ...
  \floatfoot{\textbf{Notes}: This figure reports simulate coverage probabilities for the true parameter for the binary-IV model with Bernstein polynomials of degree $k=11$ under various shape constraints.
  ...
  Number of simulations = 2000, subsampling share = $0.1\times N$.}
\end{figure}
...
Looking at the convex relaxation, for $k=2$ and $k=4$, inference is clearly too conservative as both confidence intervals have coverage probability one and are much smaller than the bootstrap interval throughout.
