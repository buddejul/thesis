...
In fact, we can use all specifications $E[YI\{Z=z\}I\{D=d\}]$, which is the sharp identified set in the sense of~\cite{mogstad2018using} (see Section~\ref{sec:general_mte}).
While it can be shown that this avoids kink issues in the constant splines model, the Bernstein polynomial model still exhibits notable kinks, as can be seen in~\ref{app_fig:id_set_binary_iv_bernstein_shape_restrictions_sharp}.
...
$\gamma_1^c = 0.5 + \beta_s/2$ and $\gamma_0^c = 0.5 - \beta_s/2$. Hence, we can plot the lower bound again as a function of $\beta_s$.

Note that kinks in this plot also correspond to points of non-differentiability in the parameter space $[-1,1]^d$.
That is, approaching a point of non-differentiability from the left and the right when viewed in terms of $\beta_s$ implies two sequences in $[-1, 1]^4$ that have result in different slopes of the value function.
...
I focus on the coverage and length around the kink. Note that away from the kink coverage should converge to $0.9$ since I use confidence intervals using $c_\alpha$ critical values (i.e.\ for a one-sided test).
Apart from this convergence issue that nominal coverage should be $95\%$.
...
While this might be easily addressed by a pre-test for $\beta_s=0$, as explained earlier, it would be desirable to have inference methods that do not depend on knowing the form of $\overline{\beta}^*(\beta_s)$ explicitly.
