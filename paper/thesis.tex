...
As shown above we can only set-identify this parameter, where the set is given by $[\underline{\beta^*}(\beta_s), \overline{\beta^*}]$.
We want to construct a random interval $CI$ such that, as $n\to\infty$ and for a given $\alpha$ and $\beta^*$,
\begin{equation*}
	P(\beta^* \in CI) \to 1-\alpha.
\end{equation*}
To that end, we can construct
