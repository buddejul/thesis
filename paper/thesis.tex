...
Lastly, the outcome is generated by $Y = Y_1 D_i + Y_0 (1 - D_i)$ where the potential outcomes are (mean-) independent of $Z_i$, so that the instrument has no direct effect on the outcome.

As shown by~\cite{vytlacil2002independence}, weak separability of~\ref{eq:treatment} and independence of $U_i, Z_i$ imply the monotonicity assumptions used in~\cite{imbens_angrist1994ecma}.
...
While the identified set is usually computed via a linear program, in the simple case considered here with a single point-identified parameter it has an intuitive and easy to compute form.
...
% TODO - Need to adjust this based on the structure of the following sections.
...
% [TODO - Maybe put a solution in the appendix similar to Bernstein figures?].
...
Because the analytical solution is non-trivial to characterize for polynomials of degree $\geq2$, I instead repeatedly solve the linear program over a range of parameter values for the complier LATE $\beta_s$.
...
% TODO: Think about this - solutions are always at corner! Equality constraints cut through space --> that's why some coefficients change.
...
For example, we can use all specifications $E[YI\{Z=z\}I\{D=d\}]$, which is the sharp identified set in the sense of~\cite{mogstad2018using} (see Section~\ref{sec:general_mte}).
...
% TODO - Explain \footnote{For example, the bounds in ~\ref{solution_cs_increasing_mtr_upper} do not exhibit a discontinuity.
%To see this, note that using all conditional moments not only fixes the difference $\gamma_1^c - \gamma_0^c$ but their levels.
%Hence, }

In this setting, the parameter space is $[-1,1]^4$, ignoring the nuisance parameter $\omega$.
...
% TODO Is Lipschitz continuity important?
...
% Note that away from the kink coverage should converge to $0.9$ since I use confidence intervals using $c_\alpha$ critical values (i.e.\ for a one-sided test).
% --> No this is not true since we don't have point-identification under these shape restrictions I think.

% TODO Provide some discussion. How useful is this? --> Analytical delta method for small cases.
...
After presenting the general MTE model setup, the rest of the paper is concerned with presenting inference approaches, that can potentially address the previously raised concerns and comparing their performance in a Monte Carlo simulation.
...
\end{align}
where $\hat{\mu}$ is the optimal value of the first-step program.
...
In this case, $\hat{\mu}$ can be used for specification tests as pointed out by~\cite{mogstad2018using} and~\cite{shea2023ivmte}.
...
\footnote{\cite{imbens2004confidence} also, however, note a uniformity issue with this confidence interval in their example: When the identified set converges to a point, using one-sided critical values will undercover.
...
Figure~\ref{fig:simulation_binary_iv_bernstein} reports the main simulation results.
